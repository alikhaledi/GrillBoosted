...
GPi_APs(1).times = [];
GPi_APs(2).times = [0.0035, 0.013, 0.10542, 0.10546, 0.10548, 0.1055, 0.10552, 0.10778, 0.1078, 0.10782, 0.10784, 0.10786, 0.10788, 0.1079]';
GPi_APs(3).times = [0.0013, 0.10542, 0.10546, 0.10548, 0.1055, 0.10552, 0.10778, 0.1078, 0.10782, 0.10784, 0.10786, 0.10788, 0.1079]';
GPi_APs(4).times = [0.0035, 0.013, 0.10542, 0.10546, 0.10548, 0.1055, 0.10552, 0.10778, 0.1078, 0.10782, 0.10784, 1.10786]';
GPi_APs = GPi_APs(1:4);
...
function [TH_APs STN_APs GPe_APs GPi_APs Striat_APs_indr Striat_APs_dr Cor_APs] = CTX_BG_TH_network(pd,corstim,tmax,dt,n,Idbs,Iappco)
    ...
    v1= -10:-1:-19; %-62+randn(n,1)*5;    % normal distribution with mean = -62/-63.8 and SD = 5
    v2= -20:-1:-29; %-62+randn(n,1)*5;
    v3= -30:-1:-39; %-62+randn(n,1)*5;
    v4= -40:-1:-49; %-62+randn(n,1)*5;
    v5= -50:-1:-59; %-63.8+randn(n,1)*5;
    v6= -60:-1:-69; %-63.8+randn(n,1)*5;
...
all=1:10;%randsample(n,n);
bll=1:10;%randsample(n,n);
cll=1:10;%randsample(n,n);
dll=1:10;%randsample(n,n);
ell=1:10;%randsample(n,n);
fll=1:10;%randsample(n,n);
gll=1:10;%randsample(n,n);
hll=1:10;%randsample(n,n);
ill=1:10;%randsample(n,n);
jll=1:10;%randsample(n,n);
kll=1:10;%randsample(n,n);
lll=1:10;%randsample(n,n);
mll=1:10;%randsample(n,n);
nll=1:10;%randsample(n,n);
oll=1:10;%randsample(n,n);

tttttemp = 0.5*ones(n,1); %rand(n,1);
gcorsna=0.3*tttttemp; %rand(n,1);
gcorsnn=0.003*tttttemp; %rand(n,1);
gcordrstr=(0.07-0.044*pd)+0.001*tttttemp; %rand(n,1);
ggege=1*tttttemp; %rand(n,1);
 
 
gsngen=zeros(n,1);
ranperind2 = [1 2]; %randperm(10,2)';
r2 = [0.25, 0.75];%rand(2,1);
gsngen(ranperind2)=0.002*r2;%rand(2,1);
gsngea=zeros(n,1);
ranperind2 = [1 2]; %ranperind2 = randperm(10,2)';
gsngea(ranperind2)=0.3*r2;%rand(2,1);
gsngi=zeros(n,1);
ranperind5 = 1:5;%randperm(10,5)';
gsngi(ranperind5)=0.15;
ggith=0.112;
ggesn=0.5;
gstrgpe=0.5;
gstrgpi=0.5;
...
if mod(size(tapers, 1), 2)
    tapers = tapers(1:end-1, :);
end
